...
function createProduct() {
    // Check if 'products' is null in localStorage
    if (localStorage.getItem('products') === null) { 
        let products = [{
                ...
                category: 'Gà rán', // Đã đổi từ 'Món mặn'
                price: 200000,
...
function createAdminAccount() {
    ...
    // Only create accounts if the 'accounts' key is not present (null) or empty (falsey in an older check)
    if (accounts === null) { 
        let accountsArray = []; // Dùng biến mới để tránh nhầm lẫn với giá trị lấy từ localStorage
        accountsArray.push({
...
// FIX: Combine both initialization calls into a single function assigned to window.onload
// This ensures BOTH createProduct() and createAdminAccount() are executed when the page loads.
